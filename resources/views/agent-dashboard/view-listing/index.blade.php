                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody id="leadResults">
                            @forelse($buyerVisitLists as $key=>$buyerVisitList)
                                <tr>
                                    <td>{{ $key + 1 + ($buyerVisitLists->currentPage() - 1) * $buyerVisitLists->perPage() }}
                                    </td>
                                    <td>{{ $buyerVisitList->listing_id }}</td>
                                    <td><a href="{{ route('view.business.listing', $buyerVisitList->listing_id) }}"
                                            target="_blank">{{ $buyerVisitList->listing->CorpName ?: $buyerVisitList->listing->DBA }}</a>
                                    </td>
                                    <td>{{ $buyerVisitList->buyer->FName }} {{ $buyerVisitList->buyer->LName }}</td>
                                    <td>{{ $buyerVisitList->buyer->Email }}</td>
                                    <td>{{ \Carbon\Carbon::parse($buyerVisitList->buyer->viewed_at)->format('M d, Y') }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
