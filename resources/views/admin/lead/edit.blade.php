...
@section('content')
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="appointment" name="appointment"
                                value="{{ $lead->AppointmentDate }}" min="{{ \Carbon\Carbon::now()->toDateString() }}">
                            @error('appointment')
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control" id="time" name="time"
                                value="{{ $lead->AppointmentTime }}" min="{{ \Carbon\Carbon::now()->toDateTimeString() }}">
                            @error('time')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="leadDate" name="leadDate"
                                value="{{ $lead->LDate }}" max="{{ \Carbon\Carbon::now()->toDateString() }}">
                            @error('leadDate')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                            ...
                            <select id="category" class="form-select" name="category">
                                ...
                                @foreach ($categoryData as $key => $data)
                                    <option value="{{ $data->CategoryID }}"
                                        {{ $lead->Category == $data->CategoryID ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select id="subCategory" class="form-select" name="subCategory">
                                ...
                                @foreach ($sub_categories as $key => $sub_cat)
                                    <option value="{{ $sub_cat->SubCatID }}"
                                        {{ $lead->SubCategory == $sub_cat->SubCatID ? 'selected' : '' }}>
                                        {{ $sub_cat->SubCategory }}</option>
                                @endforeach
                            </select>
                            @error('subCategory')
                                <small class="text-danger">{{ $message }}</small>
                            ...
                            <select id="status" class="form-select" name="status">
                                ...
                                @foreach ($status as $key => $leadStatus)
                                    <option value="{{ $leadStatus->LeadStatusID }}"
                                        {{ $lead->Status == $leadStatus->LeadStatusID ? 'selected' : '' }}>
                                        {{ $leadStatus->Status }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-check d-flex flex-column check-column p-0 m-0">
                            ...
                            <input type="checkbox" class="form-check-input" id="listed" name="listed"
                                value="1" {{ $lead->Listed == 1 ? 'checked' : '' }} onchange="changeListedValue()">
                        </div>
                    </div>
                    <!-- Address -->
                    <div class="col-md-12">
                            ...
                            <select id="state" class="form-select" name="state">
                                ...
                                @foreach ($states as $key => $value)
                                    <option value="{{ $value->State }}"
                                        {{ $lead->State == $value->State ? 'selected' : '' }}>{{ $value->StateName }}
                                    </option>
                                @endforeach
                            </select>
                            @error('state')
                            ...
                            <select id="county" class="form-select" name="county">
                                ...
                                @foreach ($counties as $key => $country)
                                    <option value="{{ $country->County }}"
                                        {{ $lead->County == $country->County ? 'selected' : '' }}>{{ $country->County }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-md-2">
                        <div class="form-check d-flex flex-column check-column p-0 m-0">
                            ...
                            <input type="checkbox" class="form-check-input" id="reInc" name="reInc"
                                value="1" {{ $lead->RealEstateInc == 1 ? 'checked' : '' }}
                                onchange="changeREInCValue()">
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="adDate" name="adDate"
                                value="{{ $lead->AdDate }}" min="{{ \Carbon\Carbon::now()->toDateString() }}">
                        </div>
                    </div>
                    <!-- SFBO -->
                    <div class="col-md-2">
                        <div class="form-check d-flex flex-column check-column p-0 m-0">
                            ...
                            <input type="checkbox" class="form-check-input" id="sfbo" name="sfbo"
                                value="1" {{ $lead->FSBO ? 'checked' : '' }} onchange="changeSFBOValue()">
                        </div>
                    </div>
                    <!-- Ad Copy -->
                    <div class="col-md-4">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#editleadForm input').on('keyup change', function() {
                $(this).valid();
            });
