...
@section('content')
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        <thead class="thead-dark">
                            <tr>
                                <th scope="col" class="checkList"><label
                                        class="custom-control custom-checkbox mb-1 align-self-center pr-4">
                                        <input type="checkbox" name="checkListing" value=""
                                ...
                                <th scope="col" class="sortable">
                                    <a
                                        href="{{ request()->fullUrlWithQuery([
                                            'sort' => 'LeadID',
                                            'direction' => sortDirection('LeadID', $currentSort, $currentDirection),
                                        ]) }}">
                                        # {!! sortIcon('LeadID', $currentSort, $currentDirection) !!}
                                    </a>
                                </th>
                                <th scope="col" class="sortable">
                                    <a
                                        href="{{ request()->fullUrlWithQuery([
                                            'sort' => 'SellerFName',
                                            'direction' => sortDirection('SellerFName', $currentSort, $currentDirection),
                                        ]) }}">
                                        Name {!! sortIcon('SellerFName', $currentSort, $currentDirection) !!}
                                    </a>
                                </th>
                                ...
                                <th scope="col" class="sortable">
                                    <a
                                        href="{{ request()->fullUrlWithQuery([
                                            ...
                                            'direction' => sortDirection('AppointmentDate', $currentSort, $currentDirection),
                                        ]) }}">
                                        Appt. Date {!! sortIcon('AppointmentDate', $currentSort, $currentDirection) !!}
                                    </a>
                                </th>
                                ...
                                <tr>
                                    <td class="checkList">
                                        <label class="custom-control custom-checkbox mb-1 align-self-center pr-4">
                                            <input type="checkbox" name="lead_id[]" value="{{ $lead->LeadID }}"
                                                class="custom-control-input listing-check">
                                            <span class="custom-control-label">&nbsp;</span>
                                        </label>
                                    </td>
                                    <td>{{ $lead->LeadID }}</td>
                                    ...
                                    <td>{{ $lead->Address }}</td>
                                    <td>{{ $lead->Phone }}</td>
                                    <td>{{ \Carbon\Carbon::parse($lead->AppointmentDate)->format('m/d/Y') }}</td>
                                    <td>{{ $lead_status[$lead->Status] ?? 'N/A' }}</td>
                                    <td class="list-btn">
                                        @if (empty($lead->AgentID))
                                            <button class="btn btn-sm" title="Assign" data-bs-toggle="modal"
                                                data-bs-target="#assignAgentModal" data-lead-id="{{ $lead->LeadID }}"
                                                data-agent-id="{{ $lead->AgentID }}">
                                                <i class="fas fa-user-plus" style="color: #6c757d;"></i>
                                            </button>
                                        @endif

                                        <a href="{{ route('show.lead', $lead->LeadID) }}"><button class="btn btn-sm"
                                                title="View">
                                                <i class="fas fa-eye"></i>
                                            </button></a>
                                        <a href="{{ route('edit.lead', $lead->LeadID) }}"> <button class="btn btn-sm"
                                                title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </button></a>
                                        <form id="delete-form-{{ $lead->LeadID }}"
                                            action="{{ route('lead.destroy', $lead->LeadID) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-sm" title="Delete"
                                                onclick="leadDelete('{{ $lead->LeadID }}')">
    ...
    <!-- Modal -->
    <div class="modal fade" id="assignAgentModal" tabindex="-1" aria-labelledby="assignAgentModalLabel"
                        ...
                        <div class="mb-3">
                            ...
                            <select name="agent_id" id="agent" class="form-select">
                                ...
                                @foreach ($agents as $agent)
                                    <option value="{{ $agent->agent_info->AgentID }}">{{ $agent->agent_info->FName }}
                                        {{ $agent->agent_info->LName }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
