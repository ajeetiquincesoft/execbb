...
@section('content')
    <div class="container-fluid content bg-light">
        <div class="row card p-4">
            <form id="addnewliststep1" action="{{ route('update.listing.step1', $listingData->ListingID) }}" method="post"
                ...
                <div class="tab" style="display: block;">
                    ...
                    <div class="row mb-2">
                            ...
                            <select id="busCategory" class="form-select" name="bus_category">
                                ...
                                @foreach ($categoryData as $key => $data)
                                    <option value="{{ $data->CategoryID }}"
                                        {{ $listingData->BusCategory == $data->CategoryID ? 'selected' : '' }}>
                            ...
                            <select id="busType" class="form-select" name="bus_type">
                                ...
                                @foreach ($sub_categories as $key => $bus_type)
                                    <option value="{{ $bus_type->SubCatID }}"
                                        {{ $listingData->SubCat == $bus_type->SubCatID ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4" style="height: 70px;">
                            ...
                            <input type="checkbox" id="franchise" class="form-check-input" name="franchise" value="1"
                                {{ $listingData->Franchise == 1 ? 'checked' : '' }} onchange="changeFranchiseValue()">
                        </div>
                    </div>
                    <div class="row mb-2">
                        <div class="col-md-4 mb-3">
                            ...
                            <select id="State" class="form-select" name="state">
                                ...
                                @foreach ($states as $key => $value)
                                    <option value="{{ $value->State }}"
                                        {{ strtolower($listingData->State) == strtolower($value->State) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select id="country" class="form-select" name="country">
                                ...
                                @foreach ($counties as $key => $value)
                                    <option value="{{ $value->County }}"
                                        {{ $listingData->County == $value->County ? 'selected' : '' }}>
                                        {{ $value->County }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-md-4" style="height: 70px;">
                            ...
                            <input type="checkbox" id="featuredListing" class="form-check-input" name="featuredListing"
                                value="1" {{ $listingData->featured == 1 ? 'checked' : '' }}
                    ...
                    <div class="row mb-2">
                        ...
                        @if (!empty($listingData->imagepath))
                            <div class="col">
                                <div id="imageUploads">
                                    <img class="view_upload_image"
                                        src="{{ asset('assets/uploads/images/' . $listingData->imagepath) }}"
                                        alt="Uploaded Image">
                                </div>
                            </div>
                        @endif
                        <div class="col-md-6">
                            ...
                            @if (isset($listingData->document_path))
                                <p class="mt-2">
                                    Current File:
                                    <a href="{{ asset('storage/' . $listingData->document_path) }}"
                    ...
                    <div class="row mb-2">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" id="city2" class="form-control" placeholder="City"
                                name="user_city" value="{{ $listingData->SCity }}">
                            @error('user_city')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            ...
                            <select id="State2" class="form-select" name="user_state">
                                ...
                                @foreach ($states as $key => $value)
                                    <option value="{{ $value->State }}"
                                        {{ strtolower($listingData->SState) == strtolower($value->State) ? 'selected' : '' }}>
                                        {{ $value->StateName }}</option>
                                @endforeach
                            </select>
                            @error('user_state')
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" id="Zip2" class="form-control" placeholder="Zip"
                                name="user_zip_code" value="{{ $listingData->SZip }}">
                            @error('user_zip_code')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                ...
                if (id) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $('#busType').append(
                            '<option value="">Select an option</option>'); // Add default option
                    ...
                    $('#second-dropdown').empty().append(
                    '<option value="">Select an option</option>'); // Reset second dropdown
                }
            });
        });
    </script>
